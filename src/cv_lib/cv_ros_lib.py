...
class ImagePublish_t:
    ...
    """
    def __init__(self,node:rclpy.Node, topic:str,queue_size:int=10):
        self._node = node
        self._topic = topic
...
class ImageConver_t:
    ...
    def update(self, image_msg:Image):
        """
        将 ROS 2 图像消息转换为 OpenCV 格式
        :param image_msg: ROS 2 图像消息
        :return: OpenCV 图像对象 (np.ndarray)
        """
        if self.use_compression:
            # 使用压缩格式
            return self.bridge.compressed_imgmsg_to_cv2(image_msg, desired_encoding="passthrough")
        else:
            # 使用未压缩格式
